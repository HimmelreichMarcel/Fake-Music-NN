...
class NeuralNetwork():
    ...
    def setRenderDevice(self, render_device = "gpu"):
        try:
            print("Set Render Device")
            print(device_lib.list_local_devices())
            if render_device == "cpu":
                num_cores = multiprocessing.cpu_count()
                config = tf.ConfigProto(intra_op_parallelism_threads=num_cores,\
                                        inter_op_parallelism_threads=num_cores, allow_soft_placement=True,log_device_placement=True,\
                                        device_count = {'CPU' : 1, 'GPU' : 0})
                session = tf.Session(config=config)
                #K.set_session(session)
                print("Set CPU")
            elif render_device == "multi_gpu":
                self._model  = multi_gpu_model(self._model, gpus=None)
                print("Set Multi GPU")
            else:
                num_cores = multiprocessing.cpu_count()
                config = tf.ConfigProto(intra_op_parallelism_threads=num_cores,\
            ...
            print("Unable to activate GPU calculation")
            print(n)
    ...
    def fitDeviceTimeTest(self, _x, _y, _batch_size=None, _epochs=1, _verbose=1):
        """ Fit the given Model"""
        """
            _x = networkinput
            _y = networkoutput
    ...
    def fit(self, _x, _y, _batch_size=None, _epochs=1, _verbose=1):
        ...
        """
        try:
            self._model = self._model.fit(x=_x, y=_y, batch_size=_batch_size, epochs=_epochs, verbose=_verbose, callbacks=self._callbacks)
        except Exception as n:
            print("Fit Model Failed!")
            print(n)
